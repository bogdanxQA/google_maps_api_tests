...
class Logger():
    # Получаем папку, где лежит текущий файл (utils)
    base_dir = os.path.dirname(os.path.abspath(__file__))
    # Поднимаемся на уровень выше (в project) и добавляем папку logs
    log_dir = os.path.join(base_dir, '..', 'logs')
    # Создаём папку, если её нет
    os.makedirs(log_dir, exist_ok=True)
    
    file_name = os.path.join(log_dir, "log_" + datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S") + ".log")
